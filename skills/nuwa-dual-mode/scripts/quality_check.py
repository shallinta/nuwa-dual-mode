...
def check_mental_models(content: str) -> tuple[bool, str]:
    """检查心智模型数量(3-10个,价值观镜片 + 操作招式两栏合计)"""
    # 匹配 ### 模型N: 或 ### N. 等模式
    models = re.findall(r'^###\s+(?:模型|Model|心智模型|招式|镜片|[A-Z]\d|[TPMV]\d)\s*\d?', content, re.MULTILINE)
    if not models:
        # fallback: 数「### 」开头的行在心智模型section中(兼容 Part A/B 分栏结构)
        in_section = False
        count = 0
        for line in content.split('\n'):
            if re.match(r'^##\s+.*(心智模型|Mental Model|key.concepts|Part\s*[AB]|价值观镜片|操作招式)', line, re.IGNORECASE):
                in_section = True
                continue
            if in_section and re.match(r'^##\s+', line) and not re.search(r'心智模型|concepts|Part\s*[AB]|价值观镜片|操作招式', line, re.IGNORECASE):
                break
            if in_section and re.match(r'^###\s+', line):
                count += 1
        if count > 0:
            passed = 3 <= count <= 10
            return passed, f"{count}个心智模型 {'✅' if passed else '❌ (应为3-10个,价值观镜片+操作招式合计)'}"
